...
program
  ...
  .option('-i|--interactive', 'Whether to automatically create the PR, or open in a browser')
  .option('-m|--message <message>', 'Commit message')
  .option('--patch-name <name>', 'Patch name')
  ...
  .action(
    async (
      ...
      targetBranch,
      additionalBranches,
      { interactive, message, patchName, security },
    ) => {
      ...
      try {
        ...
        patchName = patchName ? `${patchName}.patch` : `cherry-pick-${shortCommit}.patch`;
        ...
        for (const target of targetBranches) {
          ...
          const { data: commit } = await octokit.git.createCommit({
            ...
            tree: tree.sha,
            parents: [targetSha],
            message: message || `chore: cherry-pick ${shortCommit} from ${patchDirName}`,
          });
          ...
          if (interactive) {
            ...
            const comparePath = `${target}...${owner}:${branchName}`;
            open(
              `${repoBaseUrl}/compare/${comparePath}?${querystring.stringify({ expand: 1, body })}`,
            );
            return;
          }
